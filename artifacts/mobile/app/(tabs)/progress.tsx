...
function isActiveDay(d: { mealsLogged: number; waterLitres: number; sleepHours: number; caloriesBurned: number }) {
  return d.mealsLogged > 0 || d.waterLitres > 0 || d.sleepHours > 0 || d.caloriesBurned > 0;
}
...
export default function ProgressScreen() {
  ...
  const [activeChart, setActiveChart] = useState<"calories" | "water" | "sleep">("calories");

  const topPad = Platform.OS === "web" ? 67 : insets.top;
  const weekSummary = useMemo(() => getWeekSummary(), [getWeekSummary]);

  // ─── Aggregations from real logged data ────────────────────────────────────
  const calData = weekSummary.map((d) => ({
    label: getDayLabel(d.date),
    value: d.caloriesConsumed,
  }));
  ...
  const totalCalories = weekSummary.reduce((s, d) => s + d.caloriesConsumed, 0);
  const totalMeals = weekSummary.reduce((s, d) => s + d.mealsLogged, 0);

  const sleepDays = weekSummary.filter((d) => d.sleepHours > 0);
  const avgSleep =
    sleepDays.length > 0
      ? sleepDays.reduce((s, d) => s + d.sleepHours, 0) / sleepDays.length
      : 0;

  const waterDays = weekSummary.filter((d) => d.waterLitres > 0);
  const avgWater =
    waterDays.length > 0
      ? waterDays.reduce((s, d) => s + d.waterLitres, 0) / waterDays.length
      : 0;

  const activeDays = weekSummary.filter(isActiveDay).length;
  ...
  const chartConfig =
    activeChart === "calories"
      ? { data: calData, max: Math.max(2000, ...calData.map((d) => d.value)), color: colors.primary, unit: "kcal", title: "Calories Eaten" }
      : activeChart === "sleep"
      ? { data: sleepData, max: 12, color: "#818CF8", unit: "h", title: "Sleep Hours" }
      : { data: waterData, max: Math.max(4, ...waterData.map((d) => d.value)), color: "#38BDF8", unit: "L", title: "Water Intake" };
    ...
    <PremiumGate feature="Advanced Tracking">
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        >
          ...
          {/* ── Quick water + meals row ── */}
          <View style={styles.statsRow}>
            <StatTile
              label="Avg Water"
              value={avgWater > 0 ? `${avgWater.toFixed(1)}L` : "—"}
              icon="droplet"
              color="#38BDF8"
              bg="#E0F7FF"
            />
            ...
            <StatTile
              label="Avg Kcal/day"
              value={avgDailyCalories > 0 ? `${avgDailyCalories}` : "—"}
              unit="kcal"
              icon="bar-chart-2"
              color="#EC4899"
              bg="#FCE7F3"
            ...
            <View style={styles.consistRow}>
              <View style={styles.consistLeft}>
                ...
                <Text style={[styles.consistSub, { color: colors.mutedForeground }]}>
                  {activeDays} of 7 days active · any log counts
                </Text>
              </View>
              <Text style={[styles.consistPct, { color: colors.primary }]}>{consistency}%</Text>
            </View>
              ...
              {weekSummary.map((d, i) => {
                ...
                return (
                  <View key={i} style={styles.dayDot}>
                    ...
                    />
                    <Text style={[styles.dotLabel, { color: active ? colors.primary : colors.mutedForeground }]}>
          ...
          <View style={styles.chartTabRow}>
            {CHART_TABS.map((t) => (
              <TouchableOpacity
                key={t.key}
                onPress={() => { setActiveChart(t.key); Haptics.selectionAsync(); }}
                style={[styles.chartTab, { backgroundColor: activeChart === t.key ? t.color : colors.muted }]}
              >
                <Feather name={t.icon as any} size={13} color={activeChart === t.key ? "#fff" : colors.mutedForeground} />
                <Text style={[styles.chartTabLabel, { color: activeChart === t.key ? "#fff" : colors.mutedForeground }]}>
                  {t.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={[styles.chartCard, { backgroundColor: colors.card }]}>
            <Text style={[styles.chartTitle, { color: colors.foreground }]}>
              {chartConfig.title} — This Week
            </Text>
            {chartConfig.data.every((d) => d.value === 0) ? (
            ...
            <View style={styles.chartLegend}>
              <View style={[styles.legendDot, { backgroundColor: chartConfig.color + "55" }]} />
              <Text style={[styles.legendText, { color: colors.mutedForeground }]}>Earlier days</Text>
              <View style={[styles.legendDot, { backgroundColor: chartConfig.color, marginLeft: 12 }]} />
          ...
          <View style={[styles.weightCard, { backgroundColor: colors.card }]}>
            <View style={styles.weightHeader}>
              <View style={[styles.weightIcon, { backgroundColor: colors.peachLight }]}>
                <Feather name="activity" size={18} color={colors.peach} />
              </View>
              <View style={styles.weightInfo}>
                <Text style={[styles.weightTitle, { color: colors.foreground }]}>Weight Tracker</Text>
                {latestWeight ? (
            ...
            <View style={styles.weightInputRow}>
              <TextInput
                ...
                style={[styles.weightInput, { backgroundColor: colors.muted, color: colors.foreground }]}
              />
              <TouchableOpacity
            ...
            {weightEntries.length > 1 && (
              <>
                ...
                <Text style={[styles.histLabel, { color: colors.mutedForeground }]}>History</Text>
                  ...
                  {weightEntries
                    ...
                    .map((w, i) => (
                      <View key={i} style={styles.weightHistItem}>
                        <Text style={[styles.weightHistDate, { color: colors.mutedForeground }]}>{w.date}</Text>
                        <Text style={[styles.weightHistKg, { color: colors.foreground }]}>{w.kg} kg</Text>
                      </View>
                    ))}
                </View>
              </>
          ...
          <View style={[styles.goalsCard, { backgroundColor: colors.card }]}>
            <Text style={[styles.goalsTitle, { color: colors.foreground }]}>Weekly Goals</Text>
            <GoalRow
              label="Active Days"
              current={activeDays}
              goal={5}
            ...
            <GoalRow
              label="Avg Water / Day"
              current={parseFloat(avgWater.toFixed(1))}
              goal={2.5}
              color="#38BDF8"
              unit="L"
            />
            <GoalRow
              label="Avg Sleep"
              current={parseFloat(avgSleep.toFixed(1))}
              goal={8}
              color="#818CF8"
              unit="hrs"
            />
...
function StatTile({
  ...
  label: string; value: string; unit?: string; icon: string; color: string; bg: string;
}) {
  return (
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 14, fontFamily: "Inter_400Regular", paddingHorizontal: 20, marginBottom: 20, marginTop: 4 },
  statsRow: { flexDirection: "row", paddingHorizontal: 20, gap: 10, marginBottom: 10 },
  statTile: { flex: 1, padding: 14, borderRadius: 16, alignItems: "center", gap: 4 },
  statValue: { fontSize: 18, fontFamily: "Inter_700Bold" },
  statLabel: { fontSize: 11, fontFamily: "Inter_500Medium", textAlign: "center" },
  consistencyCard: {
    marginHorizontal: 20, borderRadius: 20, padding: 18, marginBottom: 16, gap: 12,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 2,
  },
  consistRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" },
  consistLeft: { gap: 2 },
  ...
  dayDots: { flexDirection: "row", justifyContent: "space-between", marginTop: 4 },
  dayDot: { alignItems: "center", gap: 4 },
  dot: { width: 10, height: 10, borderRadius: 5 },
  dotLabel: { fontSize: 10, fontFamily: "Inter_500Medium" },
  chartTabRow: { flexDirection: "row", paddingHorizontal: 20, gap: 8, marginBottom: 12 },
  chartTab: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 8, paddingHorizontal: 14, borderRadius: 100 },
  chartTabLabel: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
  chartCard: {
    marginHorizontal: 20, borderRadius: 20, padding: 18, marginBottom: 16,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 2, gap: 12,
  },
  chartTitle: { fontSize: 14, fontFamily: "Inter_600SemiBold" },
  emptyChart: { alignItems: "center", paddingVertical: 32, gap: 10 },
  emptyChartText: { fontSize: 13, fontFamily: "Inter_400Regular", textAlign: "center" },
  chartLegend: { flexDirection: "row", alignItems: "center", gap: 6 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 11, fontFamily: "Inter_400Regular" },
  weightCard: {
    marginHorizontal: 20, borderRadius: 20, padding: 18, marginBottom: 16,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 2, gap: 14,
  },
  weightHeader: { flexDirection: "row", alignItems: "center", gap: 12 },
  weightIcon: { width: 44, height: 44, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  ...
  weightInput: { flex: 1, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 11, fontSize: 15, fontFamily: "Inter_400Regular" },
  addWeightBtn: { width: 44, height: 44, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  divider: { height: 1 },
  histLabel: { fontSize: 12, fontFamily: "Inter_500Medium" },
  weightHistory: { gap: 8 },
  weightHistItem: { flexDirection: "row", justifyContent: "space-between" },
  weightHistDate: { fontSize: 13, fontFamily: "Inter_400Regular" },
  weightHistKg: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
  goalsCard: {
    marginHorizontal: 20, borderRadius: 20, padding: 18, marginBottom: 16,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 10, elevation: 2, gap: 16,
  },
