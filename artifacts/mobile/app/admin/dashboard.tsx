...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState<"payments" | "clients">("payments");
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.tabRow, { backgroundColor: colors.muted, borderBottomColor: colors.border }]}>
        {(["payments", "clients"] as const).map((t) => (
          <TouchableOpacity
            key={t}
          ...
          >
            <Feather
              name={t === "payments" ? "credit-card" : "users"}
              size={15}
              color={tab === t ? colors.primary : colors.mutedForeground}
            />
            <Text style={[styles.tabLabel, { color: tab === t ? colors.primary : colors.mutedForeground }]}>
              {t === "payments" ? `Payments${pending.length > 0 ? ` (${pending.length})` : ""}` : "Clients"}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={[styles.searchRow, { backgroundColor: colors.muted }]}>
        ...
        <TextInput
          ...
          placeholder={tab === "payments" ? "Search payments..." : "Search clients..."}
            ...
            filteredClients.map((c) => (
              <View key={c.id} style={[styles.clientCard, { backgroundColor: colors.card }]}>
                <View style={[styles.avatar, { backgroundColor: avatarColor(c.name) + "22" }]}>
                  <Text style={[styles.avatarText, { color: avatarColor(c.name) }]}>
                    {initials(c.name)}
                  </Text>
                </View>
                ...
                <View style={[styles.clientWeightBadge, { backgroundColor: colors.muted }]}>
                  {c.weight ? (
                    <Text style={[styles.clientWeightText, { color: colors.foreground }]}>{c.weight} kg</Text>
                  ) : (
...
function PaymentCard({
  ...
  const statusColor =
    ...
    : payment.status === "rejected" ? "#EF4444"
    : "#F59E0B";
    ...
    <View style={[styles.paymentCard, { backgroundColor: colors.card }]}>
      ...
      <View style={styles.paymentTop}>
        ...
        <View style={styles.paymentMeta}>
          ...
          {payment.userEmail ? (
            <Text style={[styles.paymentSub, { color: colors.primary }]}>{payment.userEmail}</Text>
          ) : null}
      ...
      <View style={[styles.detailGrid, { borderColor: colors.border }]}>
        <DetailItem label="Amount" value={`UGX ${payment.amount.toLocaleString()}`} colors={colors} />
        <DetailItem label="Txn ID" value={payment.transactionId} colors={colors} />
        <DetailItem label="Submitted" value={date} colors={colors} />
      </View>
...
const styles = StyleSheet.create({
  ...
  headerLabel: { color: "rgba(255,255,255,0.75)", fontSize: 12, fontFamily: "Inter_500Medium" },
  ...
  clientWeightBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10 },
  clientWeightText: { fontSize: 13, fontFamily: "Inter_600SemiBold" },
});
